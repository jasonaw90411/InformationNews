...
def analyze_with_llm(sector_data, stock_data):
    ...
    2. 根据提供的股票数据，推荐5只最具投资价值的股票，每只股票需包含：
       - 基本信息（股票代码、名称、行业）
       - **最近一日股价**（必须显示，格式：$XX.XX，**严格按照提供的current_price数值输出，不得修改**。注意：current_price是通过yfinance的ticker.info.get('currentPrice', ticker.info.get('regularMarketPrice', 0))获取的精确值，请务必使用此价格）
       - 盈利状况分析
       - 技术走势分析
       - 投资理由
       - 风险提示
    ...
    completion = openai_client.chat.completions.create(
        model=model_name,
        messages=[
            {"role": "system", "content": "你是一位经验丰富的金融分析师，专注于股票市场和板块分析。请基于提供的数据，给出专业、客观、深入的分析和建议。特别重要：在推荐个股时，**必须严格按照提供的current_price具体数值输出最近一日股价**，绝对不能自行修改、估算或使用其他价格来源。current_price是通过yfinance的ticker.info.get('currentPrice', ticker.info.get('regularMarketPrice', 0))获取的，请确保在输出中精确使用此价格值。"},
